...
require_once ( plugin_dir_path( __FILE__ ) . 'media_fields_options.php' );

 if (!class_exists('OWL_Custom_Media_Fields')) {
	
	Class OWL_Custom_Media_Fields {
    ...
    function __construct( $fields ) {
		...
		add_filter( 'attachment_fields_to_edit', array( $this, 'applyOwlFilter' ), 11, 2 );
		add_filter( 'attachment_fields_to_save', array( $this, 'saveOwlFields' ), 11, 2 );
	}
 
    public function applyOwlFilter( $form_fields, $post = null ) {
    ...
    function saveOwlFields( $post, $attachment ) {
    // If our fields array is not empty
    if ( ! empty( $this->media_fields ) ) {
...
$cmf = new OWL_Custom_Media_Fields( $owl_attchments_options );
 }